...
| `prayerDice`                | object[] | Seraph's Prayer Dice for the current session, in roll order. Each entry is `{ "value": 1-4, "spent": boolean }`; at most 16 entries. `null` leaves the dice unchanged; `[]` clears them. Rolling happens client-side — the server only stores the result. |
| `transformationCardId`      | long    | Must reference an existing TransformationCard. Ignored if `clearTransformationCard` is `true`. |
| _(no `transformationEnabled`)_ | --   | **Not settable here.** Transformations are GM-granted via `PUT /api/dh/campaigns/{campaignId}/character-sheets/{sheetId}/transformation`. If the sheet's `transformationEnabled` is `false`, any request that would **change** transformation state fails with `400 Bad Request` ("Transformations are not enabled for this character. Ask your GM to enable them."): `clearTransformationCard: true` always, and `transformationCardId`/`transformationTokens`/`wolfFormActive` when the value differs from what the sheet already holds. Restating a current value (e.g. `wolfFormActive: false` on a character that has never transformed) changes nothing and is accepted, so a multi-field body such as a rest is not rejected over a field it never moved. |
...
| `prayerDice`             | object[]                  | Yes            | Seraph's Prayer Dice for the current session, in roll order — each `{ "value": 1-4, "spent": boolean }`. Empty array for characters who have not rolled any, which is every non-Seraph. |
